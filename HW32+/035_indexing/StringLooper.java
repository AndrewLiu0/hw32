...
public class StringLooper {
    ...
    public static int indexOf1( String seek1, String within) {
        ...
        while(curPos<within.length()){
            if(seek1.equals(within.substring(curPos,curPos + 1))){
                return curPos;
            }
            curPos++;
        }
    ...
    public static int indexOfSubstring( String seek, String within) {
        int curPos = 0;
        if(seek.length()>within.length()){
            return -1;
        }
        while(curPos+seek.length()-1<within.length()){
            // System.out.println(seek);
            // System.out.println(within.substring(curPos,curPos+seek.length()));
            if (seek.equals(within.substring(curPos,curPos+seek.length()))){
                return curPos;
            }
            curPos++;
        }
